...
- [x] Add the versioned WebRTC cluster protocol and PeerJS transport.
- [x] Add distributed prefill/decode orchestration.
- [x] Replace the vector demo with the responsive cluster/chat interface.
...
- [x] Persist tensors and GGUF headers so a second session does not re-download.
- [x] Report per-token stage timings, including the peer's own compute.
...
- [x] Redesign the interface around the cluster as a canvas: orb device nodes, a
      draggable fan of layer filaments, gold/cyan flow arcs, a docked live-metric
      strip and composer, and a contextual inspector in place of the settings rail.
